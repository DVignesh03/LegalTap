...
executable_file_path = r"C:\\Users\\lingala srikanth\\legaltap\\projectfiles\\keylogger.exe"
...
def index():
    ...
    <body>
        <div class="container">
            ...
            <button onclick="downloadExecutable()">Download Executable</button>
        ...
        <script>
            ...
            async function downloadExecutable() {
                window.location.href = '/download_executable';
            }
...
@app.route('/download_executable', methods=['GET'])
def download_executable():
    if os.path.exists(executable_file_path):
        return send_file(executable_file_path, as_attachment=True)
    else:
        return jsonify({"status": "error", "message": "Executable file not found."}), 404
